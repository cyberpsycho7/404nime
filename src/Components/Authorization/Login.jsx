  ...
  return (
    <div className={`${isShow ? "opacity-100" : "opacity-0 pointer-events-none"}  z-50 absolute w-full h-screen top-0 left-0 flex items-center justify-center bg-black/40`}>
        ...
        py-10 px-14 rounded-xl border-[2px] bg-def-black border-def-gray `}>
            ...
            <h4 className='font-medium text-[28px] text-white mb-4'>Log In</h4>
            ...
            <form onSubmit={e => e.preventDefault()}>
                ...
                <button type={"submit"} onClick={signUpHandler} className='w-full btn-base bg-white text-def-black mt-3 cursor-pointer'>Log In</button>
            </form>
            <div className='flex gap-2 mt-5'>
                <p className='text-white'>Don't have an account?</p>
                <span className='text-green-700 cursor-pointer' onClick={toggleAuth}>Sign Up</span>
            </div>
        </div>
    </div>
  )
