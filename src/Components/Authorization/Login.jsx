...
const Login = ({isShow, close, toggleAuth}) => {
  ...
  return (
    <PopUpModal isShow={isShow} isLoading={isLoading} isUserLoading={user?.isLoading} close={close}>
        <h4 className='font-medium text-[28px] text-white mb-4 370res:text-xl'>Log In</h4>
        ...
        <form onSubmit={e => e.preventDefault()}>
            ...
            <button type={"submit"} onClick={signUpHandler} className='w-full btn-base bg-white text-def-black mt-3 cursor-pointer 370res:text-sm'>Log In</button>
        </form>
        <div className='flex gap-2 mt-5 items-center'>
            <p className='text-white 370res:text-sm'>Don't have an account?</p>
            <span className='text-green-700 cursor-pointer flex-shrink-0' onClick={toggleAuth}>Sign Up</span>
        </div>
    </PopUpModal>
  )
}
