...
const LongerAnimeCard = ({item}) => {
  return (
    <Link to={`${item?.lastTime ? `/watch/${item?.id}?ep=${item?.lastWatchEpisode}&time=${item?.lastTime}` : `/more-info/${item?.id}`} `}>
        ...
        500res:gap-[4px] relative -z-10 500res:p-[24px] rounded-xl`}>
            ...
            <div className='flex gap-3 items-center'>
              ...
              z-10 500res:text-[8px] 500res:leading-none 500res:p-[5px]'>{(item?.lastTime / 60).toFixed(0)
                .length <= 1
                ? `0${(item?.lastTime / 60).toFixed(0)}`
                : (item?.lastTime / 60).toFixed(0)}
              :
              {(item?.lastTime % 60).toFixed(0)
                .length <= 1
                ? `0${(item?.lastTime % 60).toFixed(0)}`
            ...
            <div className='text-lg 700res:text-base z-10 font-medium 500res:text-[10px] 500res:leading-none'>{item?.lastTime ? `${item?.type}, ${item?.releaseDate}`  : item?.type}</div>
        </div>
    </Link>
  )
}
