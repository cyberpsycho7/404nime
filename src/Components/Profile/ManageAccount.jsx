...
const ManageAccount = ({currentWidth}) => {
  ...
  const patchReq = (update = {}) => {
    // axios.patch("http://localhost:3000/users/auth/me", update, {headers: {"Authorization": `Bearer ${localStorage.getItem("JWTAccess")}`}})
    axios.patch("https://four04nime.onrender.com/users/auth/me", update, {headers: {"Authorization": `Bearer ${localStorage.getItem("JWTAccess")}`}})
    .then(resp => {
      getUserData(setUser)
  ...
  useEffect(() => {
    if(newLogin === user?.login && newPassword.length <= 0) {
